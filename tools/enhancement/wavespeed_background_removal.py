"""WaveSpeed background-removal provider."""
...
class WaveSpeedBackgroundRemoval(BaseTool):
    name = "wavespeed_background_removal"
    version = "0.1.0"
    ...
    install_instructions = (
        ...
        "config.yaml wavespeed.profiles.<profile>.background_removal.model_id."
    )
    agent_skills = ["wavespeed"]

    capabilities = ["background_removal", "cutout"]
    supports = {
    ...
    best_for = [
        "AI background/object removal through WaveSpeed",
        "clean subject cutouts with alpha matte",
    ]
    ...
    input_schema = {
        ...
        "required": [],
        "properties": {
            # Operates on a source image; no text prompt required.
            ...
            "params": {
                "type": ["object", "string"],
                "description": "JSON params merged over active profile defaults (e.g. preserve_alpha).",
            },
        },
    }
    ...
    user_visible_verification = ["Inspect cutout image and metadata sidecar"]
    ...
    def estimate_cost(self, inputs: dict[str, Any]) -> float:
        return wavespeed_estimate_cost("background_removal", inputs)
